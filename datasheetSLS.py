...
def read_chart_names(file_name):
    """ Lukee käyrien nimet ChartNimet.csv -tiedostosta """
    if not os.path.exists(file_name):
        print(f"Varoitus: {file_name} ei löydy. Käytetään oletusnimiä.")
        return []  # Palautetaan tyhjä lista, jolloin käytetään oletusnimiä
    with open(file_name, mode='r', encoding='utf-8') as file:
        reader = csv.reader(file)
        for row in reader:
            return [name.strip() for name in row]  # Palauttaa ensimmäisen rivin nimet listana
    return []


def parse_variable_values(data):
    """ Parsii muuttujat CSV-datasta avain-arvo -pareiksi. """
    variables = {}
    for row in data:
        if len(row) >= 2:  # Varmistetaan, että on tarpeeksi sarakkeita
            key = row[0].strip()
            value = row[1].strip()
            try:
                variables[key] = float(value) if '.' in value or value.isdigit() else value
            except ValueError:
                variables[key] = value  # Jos muunnos floatiksi epäonnistuu, säilytetään merkkijonona
    return variables


def parse_chart_data(data):
    """ Parsii käyrädataa ja käyttää ChartNimet.csv-tiedoston nimiä, jos saatavilla """
    chart_names = read_chart_names("ChartNimet.csv")  # Lue käyrien nimet
    chart_data = {}

    for i in range(0, len(data), 2):  
        if i + 1 < len(data):
            # Käytä luettua nimeä tai fallbackina "Chart1", "Chart2"...
            chart_name = chart_names[i // 2] if i // 2 < len(chart_names) else f"Chart{i//2 + 1}"
            chart_data[chart_name] = {
                "x": [float(x) for x in data[i]],
                "y": [float(y) for y in data[i + 1]],
            }
...
def plot_charts(chart_data, variables, show_plot=False, file_extension="png"):
    ...
    for key, data in chart_data.items():
        if "x" in data and "y" in data and len(data["x"]) == len(data["y"]):
            #plt.plot(data["x"], data["y"], label=variables.get(key, key))
...
def main():
    ...
    # Lue SLS-data erillisestä tiedostosta
    sls_data_file = './TempFiles/SLS-data-1.csv'
    if not os.path.exists(sls_data_file):
        raise FileNotFoundError(f"SLS-datatiedostoa ei löydy: {sls_data_file}")

    sls_data_raw = read_csv_file(sls_data_file)
    sls_variables = parse_variable_values(sls_data_raw)

    # Tallenna SLS-muuttujat erillisiin muuttujiin tarvittaessa
    nmax_sls = sls_variables.get("NmaxSLS")
    nmin_sls = sls_variables.get("NminSLS")
    nmmax_sls = sls_variables.get("NMmaxSLS")
    ...
    lock2T = 0 if str(variables.get("lv2")) in ["0", "0.0", "0.00"] else 1
    ...
    # Open Sans Regular koko 10 - rivitetty teksti
        ...
        {"text": str(int(variables.get("b"))), "x": base_x2, "y": 680, "font": "OpenSans-Regular", "size": 10},
        {"text": str(int(variables.get("lv"))), "x": base_x2, "y": 665, "font": "OpenSans-Regular", "size": 10},
        ...
        {"text": str(int(variables.get("fii"))), "x": base_x2, "y": 620, "font": "OpenSans-Regular", "size": 10},
        #{"text": str(variables.get("fiieff")), "x": base_x2, "y": 605, "font": "OpenSans-Regular", "size": 10},
        {"text": str(int(variables.get("fyk"))), "x": base_x2, "y": 590, "font": "OpenSans-Regular", "size": 10},
        {"text": str(int(variables.get("fck"))), "x": base_x2, "y": 575, "font": "OpenSans-Regular", "size": 10},
        ...
        {"text": "ULS", "x": base_x2-80, "y": 465, "font": "OpenSans-Regular", "size": 10},
        {"text": "SLS", "x": base_x2, "y": 465, "font": "OpenSans-Regular", "size": 10},
        ...
        {"text": str(int(variables.get("Nmax"))), "x": base_x2-80, "y": 450, "font": "OpenSans-Regular", "size": 10},
        {"text": str(int(variables.get("Nmin"))), "x": base_x2-80, "y": 435, "font": "OpenSans-Regular", "size": 10},
        {"text": str(variables.get("NMmax")), "x": base_x2-80, "y": 420, "font": "OpenSans-Regular", "size": 10},
        #{"text": str(variables.get("NM0")), "x": base_x2-80, "y": 405, "font": "OpenSans-Regular", "size": 10},

        {"text": str(int(nmax_sls)), "x": base_x2, "y": 450, "font": "OpenSans-Regular", "size": 10},
        {"text": str(int(nmin_sls)), "x": base_x2, "y": 435, "font": "OpenSans-Regular", "size": 10},
        {"text": str(nmmax_sls), "x": base_x2, "y": 420, "font": "OpenSans-Regular", "size": 10},
        #{"text": str(variables.get("NM0kNSLS")).rstrip(".0"), "x": base_x2, "y": 405, "font": "OpenSans-Regular", "size": 10},

        # Kuvateksti-osio
        {"text": "Figure 1. Main dimensions of the joint", "x": 60, "y": 445, "font": "OpenSans-Regular", "size": 8},
        {"text": "Figure 2. Normal force - Moment diagram", "x": 60, "y": 47, "font": "OpenSans-Regular", "size": 8},
    ...
    if lock2T == 1:
        ...
        texts_to_add.append({"text": str(int(variables.get("lv2"))), "x": base_x2, "y": 650, "font": "OpenSans-Regular", "size": 10})
    ...
    if LrbT == 1:
        ...
        texts_to_add.append({"text": str(int(variables.get("Lrb"))), "x": base_x2, "y": 635, "font": "OpenSans-Regular", "size": 10})
    ...
    images_to_add = [
        {"image": "./TempFiles/kuvaaja.png", "x": 10, "y": 54, "width": 580, "height": 350},
    ...
    if myyT == 1:
        outFilePDFName = (
            variables.get("Nimi")
            + " Fck " + str(variables.get("fck"))
            + " mc " + str(variables.get("myyC"))
            + " ms " + str(variables.get("myyS"))
            )
    else:  outFilePDFName = variables.get("Nimi") + " Fck " + str(variables.get("fck"))        

    add_text_and_image_to_pdf('DatasheetPohja.pdf', texts_to_add, images_to_add, outFilePDFName)

    chart_names = read_chart_names("ChartNimet.csv")
    print("Luetut käyrän nimet:", chart_names)
